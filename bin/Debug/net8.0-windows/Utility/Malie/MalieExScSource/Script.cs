    ...
    partial class Script
    {
        ...
        public void ImportMessages(string filePath)
        {
            ...
            using (var reader = File.OpenText(filePath))
            {
                ...
                while (!reader.EndOfStream)
                {
                    ...
                    // Ignore character name (no unique ID, use ImportStrings instead)
                    if (index == 0xAAAAAAAA)
                        continue;
        ...
        public void ExportStrings(string filePath)
        {
            ...
            using (var writer = File.CreateText(filePath))
            {
                ...
                {
                    var str = EscapeString(item.Value);
                    writer.WriteLine($"\u25c7{item.Key:X8}\u25c7{str}");
                    writer.WriteLine($"\u25c6{item.Key:X8}\u25c6{str}");
                    writer.WriteLine();
                }
            }
        ...
        [GeneratedRegex(@"◆(\w+)◆(.*$)")]
        private static partial Regex StrLineRegex();
